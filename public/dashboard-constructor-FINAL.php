...
<body id="bodyElement">
    ...
    <script>
        ...
        function addNewEvidence() {
            ...
            var html = '<div class="evidence-upload-item">' +
        ...
        function navigateToSection(sectionId) {
            if (sectionId === 'dashboard') {
                var placeholder = document.getElementById('section-placeholder');
                if (placeholder) placeholder.remove();
                document.querySelector('.main-container').style.display = 'block';
                return;
            }
            var names = { 'home': 'Página Principal', 'ai-tools-products': 'AI Tools + Products', 'team-room': 'Team Room' };
            var descs = { 'home': 'Página principal con información general de Latin Wave Community', 'ai-tools-products': 'Herramientas de IA y catálogo completo de productos LWC', 'team-room': 'Sala de equipo con chat, recursos y herramientas colaborativas' };
            var existing = document.getElementById('section-placeholder');
            if (existing) existing.remove();
            var html = '<div id="section-placeholder" style="position:fixed;top:0;left:0;width:100%;height:100%;background:rgba(0,0,0,0.95);backdrop-filter:blur(20px);display:flex;align-items:center;justify-content:center;z-index:1500;">' +
                ...
                '<button onclick="navigateToSection(\'dashboard\')" style="background:linear-gradient(135deg,#ffc107,#f59e0b);color:#000;border:none;padding:12px 30px;border-radius:25px;font-weight:600;cursor:pointer;">← Volver al Dashboard</button>' +
                '</div></div>';
        ...
        function initWaveSystem() {
            ...
            var videos = [document.getElementById('morningWaves'), document.getElementById('eveningWaves'), document.getElementById('nightWaves')];
        ...
        function updatePaymentFields() {
            ...
            if (method === 'paypal') { label.textContent = 'Email de PayPal'; input.placeholder = 'tu-email@paypal.com'; }
            else if (method === 'mercadopago') { label.textContent = 'Email de MercadoPago'; input.placeholder = 'tu-email@mercadopago.com'; }
            else if (method === 'btc') { label.textContent = 'Dirección BTC Wallet'; input.placeholder = 'bc1q...'; }
            else if (method === 'binance') { label.textContent = 'Binance ID'; input.placeholder = '123456789'; }
            else { label.textContent = 'Información de Pago'; input.placeholder = 'Selecciona un método de pago primero'; }
        }
        ...
        function saveProfile() {
            ...
            var assets = ['leadlightning', 'notion', 'goe1ulife', 'comizion', 'fbleadgen', 'saveclub', 'livegood', 'vitalhealth'];
            var activeAssets = 0;
            for (var i = 0; i < assets.length; i++) {
                var checkbox = document.getElementById(assets[i]);
                var input = checkbox.closest('.asset-item').querySelector('.asset-input');
                userData.digitalAssets[assets[i]] = {
                    active: checkbox.checked,
                    id: input.value,
                    verified: false
                };
                if (checkbox.checked && input.value) activeAssets++;
            }
        ...
        function generateClientsListHTML() {
            ...
            var result = '';
            var data = window.allClientsData.slice(0, 20);
            for (var i = 0; i < data.length; i++) {
                var client = data[i];
                result += '<div onclick="showClientDetails(\'' + client.name + '\')" style="background:rgba(255,255,255,0.08);border:1px solid rgba(255,193,7,0.3);border-radius:12px;padding:15px;cursor:pointer;transition:all 0.3s;">' +
                    '<div style="display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:8px;">' +
                    '<div style="color:#fff;font-weight:600;font-size:14px;">' + (i < 10 ? '⭐' : '') + ' ' + client.name + '</div>' +
        ...
        function showClientDetails(clientName) {
            var client = null;
            for (var i = 0; i < window.allClientsData.length; i++) {
                if (window.allClientsData[i].name === clientName) {
                    client = window.allClientsData[i];
                    break;
                }
            }
            if (!client) return;
        ...
        function generateAffiliatesListHTML() {
            ...
            var result = '';
            var data = window.allAffiliatesData.slice(0, 15);
            for (var i = 0; i < data.length; i++) {
                var affiliate = data[i];
                result += '<div onclick="showAffiliateDetails(\'' + affiliate.name + '\')" style="background:rgba(255,255,255,0.08);border:1px solid rgba(255,193,7,0.3);border-radius:12px;padding:15px;cursor:pointer;">' +
                    '<div style="display:flex;justify-content:space-between;margin-bottom:8px;">' +
                    '<div style="color:#fff;font-weight:600;">' + (i < 5 ? '⭐' : '') + ' ' + affiliate.name + '</div>' +
        ...
        function showAffiliateDetails(affiliateName) {
            var affiliate = null;
            for (var i = 0; i < window.allAffiliatesData.length; i++) {
                if (window.allAffiliatesData[i].name === affiliateName) {
                    affiliate = window.allAffiliatesData[i];
                    break;
                }
            }
            if (!affiliate) return;
        ...
        function generateConstructorsListHTML() {
            ...
            for (var i = 0; i < window.allConstructorsData.length; i++) {
                ...
                result += '<div onclick="showConstructorDetails(\'' + constructor.name + '\')" style="background:linear-gradient(135deg,rgba(255,193,7,0.1),rgba(0,122,255,0.05));border:2px solid rgba(255,193,7,0.3);border-radius:12px;padding:20px;cursor:pointer;">' +
                    '<div style="display:flex;justify-content:space-between;margin-bottom:10px;">' +
                    '<div style="color:#fff;font-weight:700;font-size:16px;">' + (i < 3 ? '👑' : '') + ' ' + constructor.name + '</div>' +
        ...
        function showConstructorDetails(constructorName) {
            ...
            for (var i = 0; i < window.allConstructorsData.length; i++) {
                if (window.allConstructorsData[i].name === constructorName) {
                    constructor = window.allConstructorsData[i];
                    break;
                }
            }
        ...
        document.addEventListener('click', function(e) {
            ...
            if (dropdown && avatar && !dropdown.contains(e.target) && !avatar.contains(e.target)) closeUserDropdown();
